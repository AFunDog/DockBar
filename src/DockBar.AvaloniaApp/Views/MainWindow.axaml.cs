...
internal partial class MainWindow : Window
{
    ...
    public MainWindow(MainWindowViewModel viewModel)
    {
        ...
        //
        // AddHandler(Window.WindowClosedEvent,
        //     (sender, args) => { ViewModel.Logger.Debug("{Sender} {args}", sender, args); });

        ViewModel.DockItemService.DockItemExecuted += OnDockItemExecuted;
    ...
    public void NotifyIsPanelShow()
    {
        ViewModel.HasOwnedWindow = OwnedWindows.Any(w => w.IsVisible);
    }

    private void OnLayoutUpdated(object? s, EventArgs e)
    {
        // ViewModel.Logger.Debug("MainWindow LayoutUpdate");
        ViewModel.HasOwnedWindow = OwnedWindows.Any(w => w.IsVisible);
    }
    ...
    private void OnActivated(object? sender, EventArgs e)
    {
        using var _ = LogHelper.Trace();
        ViewModel.HasOwnedWindow = OwnedWindows.Any(w => w.IsVisible);
        ViewModel.Logger.Debug("MainWindow Activate");
        // ViewModel.Logger.Verbose("IsPanelShow 属性改变 {Property} {Others}", ViewModel.IsPanelShow,
        //     new bool[]
        //     {
        //         ViewModel.IsMouseEntered, ViewModel.IsDragMode, ViewModel.IsHotKeyPressed,
        //         ViewModel.HasOwnedWindow,
        //         ViewModel.IsDockItemDraging
        //     });
    }
    ...
    private void OnDeactivated(object? sender, EventArgs e)
    {
        using var _ = LogHelper.Trace();
        ViewModel.HasOwnedWindow = OwnedWindows.Any(w => w.IsVisible);
        ViewModel.Logger.Debug("MainWindow Deactivate");
    }

    private void OnDockItemExecuted(IDockItemService sender, DockItemBase item)
    {
        if (item is DockItemFolder folder)
        {
            var folderWindow = Program.ServiceProvider.GetRequiredService<DockItemFolderWindow>();
            // GetCursorPos(out var point);
            if (DockItemList.ContainerFromItem(item.Key) is { } control && Screens.ScreenFromWindow(this) is { } screen)
            {
                var point = control.PointToScreen(new());
                folderWindow.ShowFolder(
                    folder,
                    (int)(point.X + control.Bounds.Width * screen.Scaling * 0.5),
                    (int)(point.Y + control.Bounds.Height * screen.Scaling + 12)
                );
            }
        }
    }
    ...
    private void RegisterAppHotKey()
    {
        ...
        try
        {
            if (TryGetPlatformHandle()?.Handle is { } hWnd)
                if (RegisterHotKey(new(hWnd), GetHashCode(), HOT_KEY_MODIFIERS.MOD_CONTROL, (uint)VIRTUAL_KEY.VK_SPACE))
                {
    ...
    private void UnregisterAppHotKey()
    {
        ...
        {
            if (TryGetPlatformHandle()?.Handle is { } hWnd)
                if (UnregisterHotKey(new(hWnd), GetHashCode()))
                {
        ...
        switch (msg)
        {
            ...
            case WM_TRAYICON:
                if (lParam == WM_RBUTTONUP)
                {
                    ...
                    menuWindow.ShowMenu(point.X, point.Y);
                    handled = true;
                }
    ...
    private void OnDockItemPanelDrop(object? sender, DragEventArgs e)
    {
        ...
        if (e.Data.Contains(DataFormats.Files))
        {
            ...
            foreach (var data in e.Data.GetFiles() ?? [])
            {
                ...
                if (e.Source is DockItemControl { DockItemKey: { } key } dockItemControl
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        ViewModel.Logger.Debug("MainWindow OnPointerPressed");
    ...
    private int PosXToIndex(double x)
    {
        ...
        for (var i = 0; i < ViewModel.DockItemService.Root.Count; i++)
        {
            if (x <= curRight)
                return i;
        ...
        return ViewModel.DockItemService.Root.Count;
    }
