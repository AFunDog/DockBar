...
public readonly partial record struct ColorValue([property: Key(0)] uint Rgba)
{
    ...
    public ColorValue(byte r, byte g, byte b, byte a = byte.MaxValue) : this(
        (uint)(a + (r << 24) + (g << 16) + (b << 8))
    )
    {
    }

    public static ColorValue FromArgb(uint argb) => new(((argb & 0xFF000000) >> 24) + ((argb & 0x00FFFFFF) << 8));
